...
<?php 
    ...
    $number = $_GET['number'];
    $query = "SELECT title, number, content, writer_idx, answer, secret FROM qna WHERE number = $number";
...
<body>
    <div id="board_write">
        ...
        <h2><a>글 수정</a></h2>
        <div id="write_area">
            <form action="qna_edit_ok.php?number=<?php echo $number ?>" method="post">
                <div id="in_title">
                    <textarea name="title" id="utitle" rows="1" cols="55" maxlength="100" required><?php echo "$title"; ?></textarea>
                </div>
                <div class="wi_line"></div>
                <div class="secret"><label>비밀글<input type="checkbox" name="secret" value=1 <?php if($result['secret'])echo "checked"; ?>></label></div>
